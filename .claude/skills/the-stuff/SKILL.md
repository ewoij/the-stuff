...
Server: `http://localhost:3000` (override with `$THE_STUFF_URL`)
...
```bash
curl -s http://localhost:3000/api/projects/:projectId/tasks
```
...
```bash
curl -s http://localhost:3000/api/tasks/:id
```
...
Statuses: `TODO`, `PROGRESS`, `DONE`, `ARCHIVED` (event-sourced — each change is appended).

**Change status:**
```bash
curl -s -X POST http://localhost:3000/api/tasks/:id/status \
  -H 'Content-Type: application/json' \
  -d '{"status": "DONE"}'
```
...
**List comments:**
```bash
curl -s http://localhost:3000/api/tasks/:id/comments
```

**Add comment:**
```bash
curl -s -X POST http://localhost:3000/api/tasks/:id/comments \
  -H 'Content-Type: application/json' \
...
**Update comment:**
```bash
curl -s -X PUT http://localhost:3000/api/tasks/:id/comments/:commentId \
  -H 'Content-Type: application/json' \
...
**Delete comment:**
```bash
curl -s -X DELETE http://localhost:3000/api/tasks/:id/comments/:commentId
```
...
```bash
curl -s -X POST http://localhost:3000/api/agent/next-task \
  -H 'Content-Type: application/json' \
  -d '{"projectId": 1}'
```
...
```bash
curl -s -X POST http://localhost:3000/api/agent/create-task \
  -H 'Content-Type: application/json' \
...
**Complete task:**
```bash
curl -s -X POST http://localhost:3000/api/agent/complete-task \
  -H 'Content-Type: application/json' \
...
```bash
curl -s -X POST http://localhost:3000/api/agent/release-task \
  -H 'Content-Type: application/json' \
  -d '{"taskId": 1}'
```
